...
| Wrapper native | **Capacitor 7** (`@capacitor/core`, `cli`, `android`) | Baru (kunci versi mayor, bukan "7/8") |
...
| Build Android | Gradle via Android Studio, **JDK 17**, SDK Platform 35 + Build-Tools, `minSdk 26` | Baru |
...
- Izin runtime lengkap per level API:
  ...
  - API ≤30: `ACCESS_FINE_LOCATION` untuk scan BLE (`minSdk 26` menjangkau keduanya — branching manifest + runtime wajib).
...
- `minSdk` 26 (Android 8, HP murah ter-cover); branching izin pra/pasca API 31.
