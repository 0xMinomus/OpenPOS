...
import { apiGetDashboard, apiListTransactions, type DashboardAdmin, type Trx } from '../../lib/local-api'
...
export default function Dashboard() {
  ...
  const kpis: { label: string; value: string; icon: React.ComponentType<{ className?: string }>; href?: string }[] = isAdmin
    ? [
        { label: 'Omzet Hari Ini', value: fmtRp(today.omzet), icon: ShoppingBag, href: '/laporan' },
        { label: 'Transaksi Hari Ini', value: String(today.trx_count), icon: ReceiptText, href: '/transaksi' },
        { label: 'Produk Terjual', value: String(today.items_sold), icon: Package, href: '/laporan' },
        {
          label: 'Stok Menipis',
        ...
        <div className="grid gap-4 lg:grid-cols-[1.6fr_1fr]">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              ...
              <Link to="/transaksi" className="opc-btn-sm">
                Lihat semua
              </Link>
            </CardHeader>
            <CardContent>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.6fr_1fr]">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            ...
            <Link to="/laporan" className="opc-btn-sm">
              Lihat semua
            </Link>
          </CardHeader>
          <CardContent>
